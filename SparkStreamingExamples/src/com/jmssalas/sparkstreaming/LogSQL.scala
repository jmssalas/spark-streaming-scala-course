...
package com.jmssalas.sparkstreaming
...
object LogSQL {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("LogSQL").setMaster("local[*]") //Only on Windows .set("spark.sql.warehouse.dir", "file:///C:/tmp")
    ...
    requests.foreachRDD((rdd, time) => {
      ...
      wordCountsDataFrame.write.mode("append").json("load_sql")
      
    })
    
    // Kick it off
    ssc.checkpoint("/home/jmssalas/git/spark-streaming-scala-course/checkpoint")
    ssc.start()
